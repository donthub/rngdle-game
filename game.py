...
def set_window_bounds(page, x: int, y: int, width: int, height: int, set_viewport: bool):
    ...
    # This is what actually controls window.innerWidth/innerHeight
    if set_viewport:
...
def player_game(
    ...
    with sync_playwright() as playwright:
        ...
        context = playwright.chromium.launch_persistent_context(
            ...
            viewport=None,
            args=[
                "--app=https://rngdle.com",  # removes tabs/address bar
            ],
        )
        page = context.pages[0] if context.pages else context.new_page()
        x, y = position
        set_window_bounds(page=page, x=x, y=y, width=width, height=height, set_viewport=True)
        ...
        page.locator("main").locator("> div").nth(0).locator("> div").nth(2).locator("> div").nth(1).evaluate("element => element.remove()")
...
def game_window(
    ...
    with sync_playwright() as playwright:
        ...
        page = context.pages[0] if context.pages else context.new_page()
        x, y = position
        set_window_bounds(page=page, x=x, y=y, width=width, height=height, set_viewport=False)
...
def run(config: dict):
    ...
    input()  # wait for manual exit, as before
